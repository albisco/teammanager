...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (role === Role.TEAM_MANAGER) {
    ...
    const round = await prisma.round.findUnique({ where: { id: params.id }, select: { teamId: true } });
    if (!round) return NextResponse.json({ error: "Not found" }, { status: 404 });
    const allowed = await hasStaffRole(userId, round.teamId, TeamStaffRole.TEAM_MANAGER);
